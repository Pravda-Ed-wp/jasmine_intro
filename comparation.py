...
def run():
    ...
    &emsp;&emsp;中国民族音乐调式极具特色，区别于西方的“自然大调”、“自然小调”，汉民族及绝大部分少数民族民歌使用的调式都被称为“五声调式”。五声调式以纯五度音程关系排列，由宫（C）、商（D）、角（E）、徵（G）、羽（A）作为调式的“正音”，具有主导性意义；而其余的音被称为“偏音”，其出现频率需要远小于正音，并不能作为乐句的主音与重音而出现。

    &emsp;&emsp;五声调式中，五个主音皆可作为调式主音（自然大小调式分别以C与A作为调式主音）分别被称为宫调式、商调式、角调式、徵调式、羽调式。其中，宫调式与自然大调听感相似，羽调式与自然小调听感相似；徵调式是最具有中国特色的调式，由于使用音阶的不同，听感上既可以类似于大调（如《茉莉花》》），也可以类似于小调（《沂蒙山小调》）；商调式与角调式由于终止式不明显，因此听感上缺乏结束感，而具有延续性。角调式出现最少，仅在华东地区有零星分布。
